...
public class BorrowDetailDAO {
	...
	public List<BorrowDetail> findByBookId(int bookId) throws Exception {
		...
		try {
			Session session = HibernateUtil.getSession();
			org.hibernate.Query q = session.createQuery(" from BorrowDetail d where d.book = :book");
	...
	public List<BorrowDetail> findByCustId(int customerdi) throws Exception {
		List<BorrowDetail> list = null;
		try {
			Session session = HibernateUtil.getSession();
			org.hibernate.Query q = session.createQuery(" from BorrowDetail d where d.customer = :customer");
			q.setInteger("customer", customerdi);
			list = toList(q.list());
		} catch (Exception ex) {
			throw ex;
		}
	...
	public List<BorrowDetail> findByCustName(String name) throws Exception {
		List<BorrowDetail> list = null;
		try {
			Session session = HibernateUtil.getSession();
